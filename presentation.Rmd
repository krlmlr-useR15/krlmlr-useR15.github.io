...
- Reservoir sampling (Efraimidis and Spirakis, 2006)

```{r}
dput(wrswoR::sample_int_rank)
```
...
```{r data_def, echo=F}
data <- data.frame(int=c(1, 3, 6),
                   ord = factor(letters[1:3], ordered = TRUE),
                   nom2 = factor(LETTERS[c(1,2,2)]),
                   nom3 = factor(LETTERS[3:5]))
```

```{r dependson="data_def"}
data
cluster::daisy(data, metric = "gower")
((6 - 3) / 5 + (1 / 2) + 0 + 1) / 4 # daisy(data[2:3,])
```


## From Gower to Manhattan distance

```{r dependson="data_def"}
(mdata <- mangow::mangow(data))
cluster::daisy(mdata, metric = "manhattan")
```
