...
## Run time: `size = n / 100`, `prob` linear
...
## Run time: `size = n / 10`, `prob` linear
...
```r
lm(log10(time) ~ a.+ a.:log10(n) + a.:r-1)
```
...
#m <-
  wrswoR.benchmark::timings %>%
  rename(`a.`=expr) %>%
  lm(f = log10(time)~a.+a.:log10(n)+a.:log10(n*r)-1, weights = n^2) %>%
